...
def create_webhook(arr_url: str, arr_api_key: str, webhook_url: str, arr_type: str = "sonarr", name: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(
            api_url, json=notification_data, headers=headers, timeout=10)
        response.raise_for_status()

        logger.info("Successfully created webhook for %s at %s",
                    arr_type, arr_url)
        ...
        logger.error("Error creating webhook for %s: %s",
                     arr_type, e, exc_info=True)
        return {"success": False, "message": f"Error: {str(e)}"}
...
def test_arr_connection(arr_url: str, arr_api_key: str, arr_type: str = "sonarr") -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Successfully connected to %s at %s (version %s)",
                    arr_type, arr_url, version)
        return {
            "success": True,
        ...
        logger.error("Error testing %s connection: %s",
                     arr_type, e, exc_info=True)
        return {"success": False, "message": f"Error: {str(e)}"}
