...
class RealDebridCog(commands.Cog, name="Real-Debrid"):
    ...
    async def check_premium_expiry(self) -> None:
        ...
        if not channel_id_str:
            logger.warning("RealDebrid: Could not find notification channel id in config. Cannot send expiry notifications.")
            return
        ...
        try:
            ...
            if not channel:
                logger.warning(f"RealDebrid: Could not find notification channel with ID: {channel_id}. Cannot send expiry notifications.")
                return
        except (ValueError, TypeError):
            logger.error(f"Invalid 'notification_channel_id' in config: {channel_id_str}. Must be a valid integer.")
            return
        ...
        if data and data.get('type') == 'premium':
            ...
            if premium_until_str:
                try:
                    expiry_date = datetime.fromisoformat(premium_until_str.replace('Z', '+00:00'))
                except (ValueError, TypeError):
                    logger.error(f"Invalid 'expiration' date format from Real-Debrid API: {premium_until_str}")
                    return
                ...
                logger.info(f"RealDebrid: Premium expires on {formatted_expiry_date}, {time_difference.days} days left.")

                if 0 < time_difference.days <= 7:
                    logger.info(f"RealDebrid: Sending premium expiry warning for {time_difference.days} days left.")
                    ...
                    logger.warning("RealDebrid: Sending premium expired notification.")
                    await channel.send("🔴 Your Real-Debrid premium has expired! Please renew.")
                else:
                    logger.info(f"RealDebrid: Premium is still far out ({time_difference.days} days). No notification sent.")
            else:
                logger.warning("RealDebrid: 'expiration' date not found in API response. Cannot determine expiry.")
        elif data:
            logger.info("RealDebrid: Account is not premium or type is not specified. No expiry date to track.")
        else:
    ...
    async def realdebrid_status_command(self, ctx: commands.Context) -> None:
        ...
        logger.info(f"RealDebrid: Received /realdebrid command from user {ctx.author.id}.")
        await ctx.defer(ephemeral=False)

        data = await get_realdebrid_client()

        if not data:
            await ctx.followup.send("Could not retrieve Real-Debrid account information.", ephemeral=True)
            return
        ...
        if expiration_str:
            try:
                ...
                embed.add_field(name="Premium Status", value="Active" if premium_status else "Expired", inline=True)
                embed.add_field(name="Expires In", value=f"{days_left} days", inline=True)
                embed.set_footer(text=f"Expires on {expiration_date.strftime('%Y-%m-%d %H:%M:%S UTC')}")

            except (ValueError, TypeError):
                logger.warning(f"RealDebrid: Invalid 'expiration' date format received for command: {expiration_str}")
                embed.add_field(name="Expiration Date", value="N/A (Invalid Format)", inline=False)
        else:
        ...
        logger.info(f"RealDebrid: Sent /realdebrid command response to user {ctx.author.id}.")
